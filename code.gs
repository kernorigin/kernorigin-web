...
function handleAIAssessmentAnalysis(data) {
  try {
    ...
    const answers = data.answers; // Object with cylinders
    
    // --- 1. FIXED MATH LOGIC ---
    let totalScore = 0;
    let maxPossible = 0;
    let weakest = { name: '', percent: 100 };

    // Calculate score based on actual questions answered
    for (let cyl in answers) {
        ...
        if(Array.isArray(answers[cyl])) {
            answers[cyl].forEach(a => {
                cylScore += (a.score || 0);
                cylMax += 4; // Max score per question is 4
            });
            totalScore += cylScore;
            maxPossible += cylMax;
            
            // Determine weakest cylinder by percentage
            let cylPercent = (cylScore / cylMax) * 100;
    ...
    // Final Health Score (capped at 100)
    const healthPercent = maxPossible > 0 ? Math.round((totalScore / maxPossible) * 100) : 0;

    // --- 2. GENERATE CONTENT ---
    const prompt = buildAIAssessmentPrompt(name, company, answers, weakest, healthPercent, language);
    const rawAIResponse = callGeminiAI(prompt);
    
    // --- 3. SPLIT LOGIC (Language Separation) ---
    // AI returns: [User Language Report] ---ADMIN_SUMMARY--- [English Summary]
    const parts = rawAIResponse.split('---ADMIN_SUMMARY---');
    const userAnalysis = parts[0].trim(); // This goes to PDF & User
    const adminSummary = parts.length > 1 ? parts[1].trim() : "Admin summary not generated.";

    const sanitizedAnalysis = sanitizeHtml(userAnalysis);
    
    // Generate PDF (User Language Only)
    ...
    // Log to Sheet (With Admin Summary appended for you to read)
    logAssessmentSubmission(data, healthPercent, weakest.name, adminSummary, emailSent);

    return {
        status: 'success',
        analysis: sanitizedAnalysis, // User sees this on screen
        healthScore: healthPercent
    };
...
function buildAIAssessmentPrompt(name, company, answers, weakest, healthPercent, language) {
    // Construct data string
    let dataStr = "";
    for(let cyl in answers) {
        dataStr += `\n${cyl.toUpperCase()}:\n`;
        answers[cyl].forEach(a => dataStr += `- Q: ${a.question} | A: ${a.answer} (Score: ${a.score})\n`);
    }

    let langInstruction = "Write in English.";
    if(language === 'hi') langInstruction = "Write the REPORT section in Hindi (Business Formal).";
    if(language === 'kn') langInstruction = "Write the REPORT section in Kannada (Business Formal).";

    return `
    ROLE: Senior Business Consultant.
    CLIENT: ${name} (${company}). Health Score: ${healthPercent}%. Weakest Cylinder: ${weakest.name}.
    ...
    INSTRUCTIONS:
    1. ${langInstruction}
    2. Then, output exactly "---ADMIN_SUMMARY---".
    3. Then, write a 3-bullet summary in ENGLISH for the internal admin team.

    FORMAT FOR REPORT (HTML only, no markdown):
    <h3>Executive Summary</h3>
    <p>[Diagnosis in ${language}]</p>
    <h3>Primary Bottleneck: ${weakest.name}</h3>
    <p>[Root cause analysis in ${language}]</p>
    <h3>Recommendations</h3>
    <p>[Next steps in ${language}]</p>
...
// ============================================
// FUNCTION 1: buildAIAssessmentPrompt
// ============================================
function buildAIAssessmentPrompt(name, company, answers, scores, weakest, healthPercent, language) {
  ...
  let langInstruction = "";
  let requiresAdminSummary = false;
  
  if (language === 'hi') {
    requiresAdminSummary = true;
    langInstruction = `
...
2. AFTER the Hindi report, add this ENGLISH section for internal team:
   <hr>
   <h3>INTERNAL DIAGNOSTIC SUMMARY (ENGLISH - NOT FOR CLIENT)</h3>
   <ul>
   <li><strong>Primary Bottleneck:</strong> ${weakest.name} (${weakest.score}/8) - [One sentence diagnosis]</li>
   <li><strong>Business Context:</strong> [One sentence about their industry/company]</li>
   <li><strong>Recommended Pitch:</strong> [Which service to sell them and why]</li>
   <li><strong>Key Quote:</strong> [One specific thing they said that reveals the core issue]</li>
   </ul>
`;
  } else if (language === 'kn') {
    requiresAdminSummary = true;
...
2. AFTER the Kannada report, add this ENGLISH section for internal team:
   <hr>
   <h3>INTERNAL DIAGNOSTIC SUMMARY (ENGLISH - NOT FOR CLIENT)</h3>
   <ul>
   <li><strong>Primary Bottleneck:</strong> ${weakest.name} (${weakest.score}/8) - [One sentence diagnosis]</li>
...
`;
    ...
    langInstruction = "Write the entire report in professional US English.";
  }
...
<p>Based on your responses, ${weakest.name} is your weakest cylinder (${weakest.score}/8 score). Here's what this means and why it matters.</p>
...
<p>Match their situation to one of the 3 case studies above.</p>
...
<h3>Next Steps</h3>
<ol>
<li>Book a 30-min debrief call</li>
<li>Explore the recommended diagnostic</li>
<li>Share with your leadership team</li>
</ol>

${requiresAdminSummary ? '\nREMEMBER: After the main report, add the INTERNAL DIAGNOSTIC SUMMARY section in English as specified above.' : ''}
...
LENGTH: 500-700 words for main report + 100 words for admin summary if non-English.
`;
...
// ============================================
// FUNCTION 2: createAssessmentPDF
// ============================================
...
function logAssessmentSubmission(data, healthScore, primaryBottleneck, analysis, emailSent) {
  try {
    ...
    Logger.log(`✅ Assessment logged: ${data.name} (${healthPercent}%)`);
